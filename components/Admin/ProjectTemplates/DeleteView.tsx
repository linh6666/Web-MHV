...
const DeleteView = ({ idItem, onSearch }: DeleteProductProps) => {
  const handleDeleteUsers = async () => {
    try {
      ...
      notifications.show({
        title: "Thành công",
        message: `${idItem.length} loại dự án đã được xoá`,
        color: "green",
      ...
      console.error("Lỗi xoá loại dự án:", error);

      notifications.show({
        title: "Lỗi",
        message: "Đã xảy ra lỗi khi xoá loại dự án.",
        color: "red",
        icon: <IconX size={20} />,
      });
    }
  ...
  return (
    <div>
      <Text size="lg" fw={500} mb="md">
        Bạn có chắc chắn muốn xóa {idItem.length} loại dự án đã chọn?
      </Text>
