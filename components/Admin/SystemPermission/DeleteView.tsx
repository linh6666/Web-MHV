...
const DeleteView = ({ idItem, onSearch }: DeleteProductProps) => {
  const handleDeleteUsers = async () => {
    try {
      ...
      notifications.show({
        title: "Thành công",
        message: `${idItem.length} cấu hình đã được xoá`,
        color: "green",
      ...
      console.error("Lỗi xoá cấu hình:", error);

      notifications.show({
        title: "Lỗi",
        message: "Đã xảy ra lỗi khi xoá cấu hình.",
        color: "red",
        icon: <IconX size={20} />,
      });
    }
  ...
  return (
    <div>
      <Text size="lg" fw={500} mb="md">
        Bạn có chắc chắn muốn xóa {idItem.length} cấu hình đã chọn?
      </Text>
